...
PLUGIN_NAME=$(basename "${CLAUDE_PLUGIN_ROOT:-vygotsky-knowledge}")
...
const PLUGIN_NAME = process.env.VYGOTSKY_PLUGIN_NAME || 'vygotsky-knowledge';
...
const MASTERY_TYPES = new Set(['prediction', 'explanation', 'transfer', 'directive', 'design_decision', 'disagreement', 'argument_choice', 'source_integration', 'structural_decision']);
...
if (eventType === 'compact') {
  const msg = 'Vygotsky session resumed after compaction. ' +
    'Read the diary files in ~/.vygotsky/diary/ to re-orient on the thinker model. ' +
...
const lines = ['## Thinker Session Brief'];
...
// Developer summary
const devSummary = readSummary('thinker');
if (devSummary) lines.push('\\n**Thinker model:** ' + devSummary.slice(0, 200));
...
for (const cf of conceptFiles) {
  ...
  if (hasMastery) {
    for (const t of ['transfer', 'directive', 'design_decision', 'argument_choice', 'structural_decision', 'source_integration', 'prediction', 'explanation', 'disagreement']) {
...
if (!conceptFiles.length) {
  lines.push('\\n_New thinker — no observations yet. Start in senior_peer posture._');
} else if (edgeCount > 0) {
...
// --- Assemble ---
const parts = ['You are Vygotsky — a theory-building knowledge work partner.\\n', skill, '\\n---\\n', brief];
